...
function channel1_handler(argv, port1) {
    ...
    switch(argv.data.msg) {
        ...
        case "CH1/(3)BOP_INIT":
            ...
            fetch(base_url+"recent.conf").then(res => res.text())
            .then(txt => { // ex) 20241128
                if(txt.length != 8) { console.error("recent.conf 파일에 오류가 있습니다."); return false; }
                ...
                Promise.all([fetch_y, fetch_m, fetch_d, fetch_f]).then(data => { 
                    // console.log("!!! data : ", data);
                    let y = _extract_url(data[0]);
                    let m = _extract_url(data[1]);
                    let d = _extract_url(data[2]);
                    let f = _extract_url(data[3]);
                    let y_list = y.filter(_d => /^[0-9]{4}\/$/.exec(_d) );
                    let m_list = m.filter(_d => /^[0-9]{2}\/$/.exec(_d) );
                    let d_list = d.filter(_d => /^[0-9]{2}\/$/.exec(_d) );
                    let f_list = f.filter(_d => /.*\.csv$/.exec(_d) );
                    console.log("!!! y : ", y, y_list);
                    console.log("!!! m : ", m, m_list);
                    console.log("!!! d : ", d, d_list);
                    console.log("!!! f : ", f, f_list);
        ...
        case "GET_STACK__STREAM":
            ...
            url_lists.then(url_arr => {
                ...
                var mat_arr = url_arr.map(url => {
                    ...
                    // 1.1 e단위 변환 및 y값 반전
                    .then(txt => {
                        return _parse_imp_data(txt, (arr) => {
